...
oauth =
	authorization: (url, parameters, key, secret, utcOffset) ->
		oauthParameters =
		...
		oauth.sign(url, lazy(parameters).extend(oauthParameters).toObject(), secret).map((_) -> Object.freeze(
	...
	sign: (url, parameters, secret) ->
		switch parameters.oauth_signature_method
			when 'HMAC-SHA1'
				bilby.some(oauthsign.hmacsign('POST', url, parameters, secret))
			else bilby.none
